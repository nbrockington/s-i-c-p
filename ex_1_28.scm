...
; alternate form of Fermat's Little Theorem, which states that if n is
; a prime number and a is any positive integer less than n, then a
...
; in expmod, we check to see if we have discovered a ``nontrivial
; square root of 1 modulo n,'' that is, a number not equal to 1 or n -
; 1 whose square is equal to 1 modulo n. It is possible to prove that
; if such a nontrivial square root of 1 exists, then n is not
...
; of 1, and use this to implement the Miller-Rabin test with a
; procedure analogous to fermat-test. Check your procedure by testing
; various known primes and non-primes. Hint: One convenient way to
...
(define (expmod-signal base exp m)
  (cond ((= exp 0) 1)
        ((even? exp)
           (square-mod-with-check (expmod-signal base (/ exp 2) m) m))
        (else
         (remainder (* base (expmod-signal base (- exp 1) m))
                    m))))


(define (square-mod-with-check x m)
   (define (check-nontrivial-sqrt square-x)
      (if (and (= square-x 1)
          (not (= x 1))
          (not (= x (- m 1))))
      0
      square-x))
   (check-nontrivial-sqrt (remainder (square x) m)))
...
; Inferior procedure that unnecessarily implemented expmod process
; twice when exp was even:

; (define (expmod-signal base exp m)
;  (cond ((= exp 0) 1)
;        ((even? exp)
;           (if (and (= (remainder (square (expmod-signal base (/ exp 2) m)) m) 1)
;                    (not (= base 1))
;                    (not (= base (- m 1))))
;               0 
